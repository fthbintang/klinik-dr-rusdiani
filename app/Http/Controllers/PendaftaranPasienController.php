...
class PendaftaranPasienController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $words = explode(' ', $namaPoli);
            if (count($words) > 1) {
                $inisialPoli = strtoupper(substr($words[1], 0, 4));
            } else {
                $inisialPoli = strtoupper(substr($words[0], 0, 3));
            }
            ...
            // Kirim email sesuai form hidden
            Mail::to($validatedData['email'])->send(new SendEmailDaftarPasien($pasien, $validatedData));
